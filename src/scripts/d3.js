...
var histogram = function() {
  ...
  function histogram(data) {
    ...
    // Assign dataFromFile to bins by value, ignoring any outside the domain.
    for (i = 0; i < n; ++i) {
      x = values[i];
      if (x0 <= x && x <= x1) {
...
function bindIndex(parent, group, enter, update, exit, data) {
  ...
  // Put any remaining dataFromFile into enter.
  for (; i < dataLength; ++i) {
    if (node = group[i]) {
...
function bindKey(parent, group, enter, update, exit, data, key) {
  ...
  // Add any remaining nodes that were not bound to dataFromFile to exit.
  for (i = 0; i < groupLength; ++i) {
...
var pie = function() {
  ...
  function pie(data) {
    ...
    // Optionally sort the arcs by previously-computed values or by dataFromFile.
    if (sortValues != null) index.sort(function(i, j) { return sortValues(arcs[i], arcs[j]); });
    else if (sort != null) index.sort(function(i, j) { return sort(data[i], data[j]); });

    // Compute the arcs! They are stored in the original dataFromFile's order.
